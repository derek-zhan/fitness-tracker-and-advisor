...
export async function GET(request: Request) {
  ...
  const importFlow = url.searchParams.get("flow") === "import";
  ...
  const workoutDay = importFlow ? 0 : program === "glute6" ? 100 + day : day;
  ...
  const params = new URLSearchParams({
    ...
    scope: `openid email https://www.googleapis.com/auth/spreadsheets${importFlow?" https://www.googleapis.com/auth/drive.metadata.readonly":""}`, state,
